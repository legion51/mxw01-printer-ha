...
INTERNAL_URL = f"http://{os.getenv('HOSTNAME', 'local_mxw01_printer').replace('_', '-')}:8099"
driver = MXW01Driver()
...
async def status() -> dict:
    return {"connected": driver.connected, "device_address": driver.address or settings["device_address"] or None, "device_name": settings["device_name"], "bridge_url": INTERNAL_URL}
...
<div class="grid"><section class="card"><h2>Макет печати</h2><label>Заголовок<input id="title" placeholder="Например: Заказ № 123"></label><label>Основной текст<textarea id="body" placeholder="Введите текст чека или сообщения. Каждая новая строка будет напечатана отдельно."></textarea></label><label class="toggle"><input id="center" type="checkbox"> Выровнять основной текст по центру</label><label class="toggle"><input id="separator" type="checkbox" checked> Добавить разделительную линию перед QR-кодом и картинкой</label><label>Данные для QR-кода<input id="qrdata" placeholder="Ссылка, номер заказа или любой текст"></label><label>Ссылка на картинку<input id="imageurl" type="url" placeholder="https://example.org/image.png"></label><div class="row"><label>Шрифт<input id="font" type="number" min="10" max="40" value="20"></label><label>Размер QR<input id="qr" type="number" min="1" max="10" value="3"></label><label>Масштаб картинки<input id="scale" type="number" min="20" max="200" value="100"></label></div><p class="hint">Поля необязательны: заполните только то, что хотите напечатать. Технические теги вводить не нужно.</p><div class="buttons"><button onclick="preview()">Предпросмотр</button><button class="primary" onclick="printDoc()">Напечатать</button></div><p id="message" class="hint"></p></section>
<section class="card"><h2>Принтер</h2><label>Имя при поиске<input id="name" value="MXW01"></label><div class="row"><label>Bluetooth-адрес<select id="address"><option value="">Выберите через поиск</option></select></label><button onclick="scan()">Найти</button></div><div class="buttons"><button onclick="save()">Сохранить принтер</button></div><p class="hint">Адрес сохраняется в данных add-on и восстанавливается после перезапуска.</p><p class="hint">Адрес для integration: <code id="bridgeurl">загрузка…</code></p><h2>Предпросмотр</h2><img id="preview" alt="Здесь появится макет печати"><p class="hint">Всё готово к печати после появления статуса «подключён».</p></section></div></main>
<script>
...
function payload(){let lines=[],title=$('title').value.trim(),body=$('body').value.trim(),qr=$('qrdata').value.trim(),image=$('imageurl').value.trim();if(title)lines.push('# '+title);if(body){let prefix=$('center').checked?'[C]':'';lines.push(...body.split(/\r?\n/).map(line=>line.trim()?prefix+line:''))}if($('separator').checked&&(qr||image))lines.push('---');if(qr)lines.push('[QR:'+qr+']');if(image)lines.push('[IMG:'+image+']');return{markdown:lines.join('\n'),font_size:+$('font').value,qr_size:+$('qr').value,image_scale:+$('scale').value}}
...
async function refresh(){try{let s=await (await api('status')).json();$('state').textContent=s.connected?'подключён':'не подключён';$('state').className='status '+(s.connected?'on':'');$('bridgeurl').textContent=s.bridge_url}catch(e){$('state').textContent='нет связи'}}
